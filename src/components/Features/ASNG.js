...
function ASNG() {
  ...
  useEffect(()=>{
    fetch("https://intermeterflaskserver.herokuapp.com/pred").then(
      res=>res.json()
    ).then(
      data => {
        setData(data)
        ...
        console.log(data)
        console.log(newlen)
        const listtoadd = {"resource": data["data"]["resource"], "announcements": data["data"]["updates"][0]["announcements"], "starttime": data["data"]["updates"][0]["starttime"]}
        const listtoadd1 = {"resource": data["data"]["resource"], "announcements": data["data"]["updates"][1]["announcements"], "starttime": data["data"]["updates"][1]["starttime"]}
